...
export function BrandLaunchKit({ initialDomain = "", initialVisualStyle = "Modern" }: { initialDomain?: string; initialVisualStyle?: string }) {
  ...
  return <section className="text-white">
    ...
    {authRequired && <div className="rounded-2xl border border-[#D4AF37]/25 bg-[#D4AF37]/[.05] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#D4AF37]">Pro access required</p><h2 className="mt-3 text-2xl font-bold">Sign in to continue your Brand Launch Kit.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">Brand Launch is available to active Pro and trial accounts, with ten kits each month.</p><div className="mt-6 flex flex-wrap gap-3"><Link href={`/sign-in?redirect=${authDestination}`} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#D4AF37] px-5 text-sm font-bold text-black">Sign in</Link><Link href="/pricing?source=brand-launch" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/15 px-5 text-sm font-semibold text-white">View Pro</Link></div></div>}
    {overview ? <>
