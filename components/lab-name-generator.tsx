...
export function LabNameGenerator({ internalTools = false, founderSignalEnabled = false }: { internalTools?: boolean; founderSignalEnabled?: boolean }) {
  ...
  async function scoreCandidate(candidate: NameSprintCandidate) {
    if (!founderSignalEnabled) {
      setScoreErrors((current) => ({ ...current, [candidate.id]: "Founder Signal is available with NamoLux Pro." }))
      return
    }
  ...
  return (
    <main className="min-h-screen bg-[#08090a] px-4 pb-20 pt-28 text-stone-100 sm:px-6">
      <section className="mx-auto max-w-6xl">
        ...
        {stage === "results" && run && <ResultsScreen run={run} grouped={grouped} saved={saved} rejecting={rejecting} founderSignals={founderSignals} founderSignalEnabled={founderSignalEnabled} scoringCandidate={scoringCandidate} scoreErrors={scoreErrors} onScore={scoreCandidate} onSave={toggleSave} onRejecting={setRejecting} onReject={rejectCandidate} onCompare={() => setStage("compare")} onAudit={internalTools ? () => setStage("audit") : null} onRefine={generateNames} refining={busy === "generate"} notice={notice} />}
        {stage === "compare" && run && <CompareScreen candidates={savedCandidates} founderSignals={founderSignals} founderSignalEnabled={founderSignalEnabled} onBack={() => setStage("results")} />}
...
function ResultsScreen({ run, grouped, saved, rejecting, founderSignals, founderSignalEnabled, scoringCandidate, scoreErrors, onScore, onSave, onRejecting, onReject, onCompare, onAudit, onRefine, refining, notice }: { run: Run; grouped: Array<{ territory: SemanticTerritory; candidates: NameSprintCandidate[] }>; saved: string[]; rejecting: string | null; founderSignals: Record<string, OnDemandFounderSignal>; founderSignalEnabled: boolean; scoringCandidate: string | null; scoreErrors: Record<string, string>; onScore: (candidate: NameSprintCandidate) => void; onSave: (candidate: NameSprintCandidate) => void; onRejecting: (id: string | null) => void; onReject: (candidate: NameSprintCandidate, reason: string) => void; onCompare: () => void; onAudit: (() => void) | null; onRefine: () => void; refining: boolean; notice: string }) {
  return <section>
...
function CandidateCard({ candidate, signal, founderSignalEnabled, scoring, scoreError, saved, rejecting, onScore, onSave, onRejecting, onReject }: { candidate: NameSprintCandidate; signal: OnDemandFounderSignal | null; founderSignalEnabled: boolean; scoring: boolean; scoreError: string; saved: boolean; rejecting: boolean; onScore: () => void; onSave: () => void; onRejecting: () => void; onReject: (reason: string) => void }) {
  ...
  return <article className="border border-white/12 bg-[#0d0e0f] p-5 sm:p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-2">{founderSignalEnabled ? <button type="button" onClick={onScore} disabled={scoring || Boolean(signal)} className={signal ? secondary : primary}>{scoring ? <LoaderCircle className="animate-spin" size={15} /> : <Sparkles size={15} />}{scoring ? "Scoring" : signal ? `Founder Signal ${signal.score}` : "Run Founder Signal"}</button> : <a href="/pricing?reason=founder-signal-pro&from=name-sprint" className={primary}><Sparkles size={15} /> Unlock Founder Signal</a>}<button type="button" onClick={onSave} className={saved ? primary : secondary}>{saved ? <Check size={15} /> : <Save size={15} />}{saved ? "Saved" : "Save"}</button><button type="button" onClick={onRejecting} className={secondary}><X size={15} /> Reject</button></div>
