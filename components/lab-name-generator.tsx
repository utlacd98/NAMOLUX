...
function CandidateCard({ candidate, saved, rejecting, onSave, onRejecting, onReject, onPreference }: { candidate: NameSprintCandidate; saved: boolean; rejecting: boolean; onSave: () => void; onRejecting: () => void; onReject: (reason: string) => void; onPreference: (type: NameFeedbackType, message: string) => void }) {
  const availableDomain = candidate.domainStatuses.find((domain) => domain.status === "available")
  const fullDomain = availableDomain ? `${candidate.normalizedName}.${availableDomain.tld}` : null
  ...
  return <article className="border border-white/12 bg-[#0d0e0f] p-5 sm:p-6">
    ...
    {fullDomain && <div className="mt-4 flex flex-wrap items-center gap-3 border border-emerald-400/20 bg-emerald-500/[0.05] p-3"><div className="min-w-0 flex-1"><p className="text-sm font-medium text-emerald-200">{fullDomain} is available</p><p className="mt-0.5 text-xs text-stone-500">Confirm the final price and availability at Namecheap.</p></div><a href={namecheapLink(fullDomain, { source: "name_sprint", content: availableDomain?.tld })} target="_blank" rel="sponsored noopener noreferrer" className={primary} aria-label={`Buy ${fullDomain} on Namecheap`}>Buy on Namecheap <ExternalLink size={15} /></a></div>}
