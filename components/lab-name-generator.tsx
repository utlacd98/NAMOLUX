  ...
  return (
    <main className="min-h-screen bg-[#08090a] px-4 pb-20 pt-28 text-stone-100 sm:px-6">
      <section className="mx-auto max-w-6xl">
        ...
        {stage === "results" && run && <ResultsScreen run={run} grouped={grouped} saved={saved} rejecting={rejecting} founderSignals={founderSignals} founderSignalEnabled={founderSignalEnabled} scoringCandidate={scoringCandidate} scoreErrors={scoreErrors} onScore={scoreCandidate} onSave={toggleSave} onRejecting={setRejecting} onReject={rejectCandidate} onPreference={preference} onCompare={() => setStage("compare")} onAudit={internalTools ? () => setStage("audit") : null} onRefine={generateNames} refining={busy === "generate"} notice={notice} />}
...
function CandidateCard({ candidate, signal, founderSignalEnabled, scoring, scoreError, saved, rejecting, onScore, onSave, onRejecting, onReject, onPreference }: { candidate: NameSprintCandidate; signal: OnDemandFounderSignal | null; founderSignalEnabled: boolean; scoring: boolean; scoreError: string; saved: boolean; rejecting: boolean; onScore: () => void; onSave: () => void; onRejecting: () => void; onReject: (reason: string) => void; onPreference: (type: NameFeedbackType, message: string) => void }) {
