...
export function SocialProof() {
  ...
  return (
    <section className="border-y border-border/30 bg-muted/20 py-20" aria-label="Social proof">
          ...
          <div className="mx-auto mt-10 flex max-w-3xl items-center justify-center">
            {stats.map((stat, index) => (
              <div key={stat.label} className="flex items-center">
                <div
                  className={cn("px-8 py-2 text-center opacity-0 lg:px-12", isVisible && "animate-fade-up")}
                  style={{
                ...
                {/* Divider line */}
                {index < stats.length - 1 && <div className="h-12 w-px bg-border/50" aria-hidden="true" />}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="overflow-hidden">
            <div className="animate-marquee flex gap-16 whitespace-nowrap">
              {[...fictionalLogos, ...fictionalLogos].map((logo, index) => (
                <div
                  key={`${logo}-${index}`}
                  className="flex items-center gap-2 text-base font-medium tracking-wide text-muted-foreground/40 transition-colors hover:text-muted-foreground/60"
                >
