...
export const dnsCloudflareProvider: AvailabilityProvider = {
  name: "dns_cloudflare_ns",
    ...
    try {
      ...
      if (!response.ok) {
        return {
          domain,
          available: false,
          provider: "dns_cloudflare_ns",
          latencyMs: Date.now() - started,
          confidence: "low",
          error: `cf_status_${response.status}`,
        }
      }
      ...
      return {
        ...
        latencyMs: Date.now() - started,
        confidence: "low",
        error: `cf_unknown_status_${data.Status}`,
      }
    } catch (error: any) {
      return {
        domain,
        ...
        latencyMs: Date.now() - started,
        confidence: "low",
        error: error?.message || "cf_dns_error",
      }
    }
  },
}
